    ...
    case 'listar':
     ...
     $rspta=$clientes->listar($fecha_inicio,$fecha_fin);
        ...
        while($reg=$rspta->fetch_object()){ //recorre los registros de la tabla
            ...
            $data[]=array(
                "0"=>($reg->estado_pago == 'Pendiente')?'<button class="btn btn-warning" onclick="mostrar('.$reg->id_pago.')"><i class="fa fa-pencil-square-o"></i></button>':
                '<button class="btn btn-warning" onclick="mostrar('.$reg->id_pago.')"><i class="fa fa-pencil-square-o"></i></button>'.
